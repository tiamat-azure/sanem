...
| Volume               | `video.volume` + coupure du son, persisté en `localStorage`. Flèches haut/bas : ±0,05. Curseur volume focalisé : gauche/droite restent le nudging natif du `range`. <kbd>Contrôle</kbd>+<kbd>flèche en bas</kbd> coupe le son, <kbd>Contrôle</kbd>+<kbd>flèche haut</kbd> le réactive. Clic mute suit l'état *silenced* (`muted` ou volume 0). Si le volume est à 0 au moment de réactiver, restaurer le dernier niveau non nul (sinon un pas de 0,05). `sanem-volume` ne stocke pas 0 : dernier niveau audible + `sanem-muted`. Tooltips `.has-tip` sans `title` natif : mute via `aria-label` « Couper le son (raccourci : Contrôle + flèche en bas) » / « Réactiver le son (raccourci : Contrôle + flèche haut) » ; un seul tooltip sur le wrap volume, peint depuis `data-tip` « Volume (raccourci : flèche haut / flèche en bas) » (le wrap n'a pas d'`aria-label`) ; le curseur reste le seul contrôle nommé « Volume ». |
