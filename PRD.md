...
| -------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Lire / Pause         | Bouton principal, seul élément en blanc plein de la barre. Touche <kbd>Espace</kbd> sans modificateur (l'autorepeat ne bascule pas lecture/pause). <kbd>Contrôle</kbd> / <kbd>Cmd</kbd> / <kbd>Alt</kbd>+<kbd>Espace</kbd> restent au navigateur / IME. |
| Recul / avance 10 s  | Pas de boutons dédiés dans la barre (redondants). `currentTime ± 10` via flèches gauche/droite et double-tap sur les tiers latéraux. <kbd>Alt</kbd> / <kbd>Contrôle</kbd> / <kbd>Cmd</kbd>+flèches restent l'historique du navigateur. |
...
| Volume               | `video.volume` + coupure du son, persisté en `localStorage`. Flèches haut/bas : ±0,05. Curseur volume focalisé : gauche/droite restent le nudging natif du `range`. <kbd>Contrôle</kbd> ou <kbd>Cmd</kbd>+<kbd>flèche en bas</kbd> coupe le son, <kbd>Contrôle</kbd> ou <kbd>Cmd</kbd>+<kbd>flèche haut</kbd> le réactive. Clic mute suit l'état *silenced* (`muted` ou volume 0). Si le volume est à 0 au moment de réactiver, restaurer le dernier niveau non nul (sinon un pas de 0,05). `sanem-volume` ne stocke pas 0 : dernier niveau audible + `sanem-muted`. Tooltips `.has-tip` sans `title` natif : mute via `aria-label` « Couper le son (raccourci : Contrôle ou Cmd + flèche en bas) » / « Réactiver le son (raccourci : Contrôle ou Cmd + flèche haut) » ; un seul tooltip sur le wrap volume, peint depuis `data-tip` « Volume (raccourci : flèche haut / flèche en bas) » (le wrap n'a pas d'`aria-label`) ; le curseur reste le seul contrôle nommé « Volume ». |
...
| Épisode précédent / suivant | Icônes skip-back / skip-forward, sans texte visible. <kbd>Page précédente</kbd> / <kbd>Page suivante</kbd> empruntent le même chemin que les boutons, no-op s'il n'y a pas de voisin — y compris lorsque le curseur volume a le focus (`preventDefault`, pas de paging natif du `range`). Avec <kbd>Contrôle</kbd> / <kbd>Cmd</kbd> / <kbd>Alt</kbd> / <kbd>Maj</kbd>, les touches Page restent au navigateur (changement d'onglet). Tooltips « Épisode précédent (raccourci : Page précédente) » / « Épisode suivant (raccourci : Page suivante) ». Le précédent est masqué sur le premier épisode et hors série. Chip bas-droit en fin d'épisode : mêmes icônes, précédent à gauche du suivant. |
| Plein écran          | Fullscreen API **sur le conteneur**, jamais sur `<video>`, sinon la barre maison disparaît. Touche <kbd>F</kbd> sans modificateur, et <kbd>Échap</kbd>. <kbd>Contrôle</kbd> / <kbd>Cmd</kbd> / <kbd>Alt</kbd>+<kbd>F</kbd> restent au navigateur (Rechercher). Icône d'entrée hors plein écran, icône de sortie (flèches rentrantes) une fois en plein écran **natif ou overlay**. Tooltips « Plein écran (raccourci : F) » / « Quitter le plein écran (raccourci : F) ». Sur téléphone : si l'API ou `orientation.lock` échoue, repli CSS qui étend la vidéo sur le grand côté (paysage). |
